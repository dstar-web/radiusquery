...
std::unordered_map<int, LinkedList> radiusQuery(const std::vector<std::vector<double>>& points, double radius, const std::vector<double>& reference, const std::vector<int>& queryIndices) {
    ...
    for (size_t i = 0; i < dist.size(); ++i) {
        int idx_i = dist[i].second;
        for (size_t j = i + 1; j < dist.size(); ++j) {
            if (dist[j].first - dist[i].first > radius) {
                break;
            }
            int idx_j = dist[j].second;
            if (distance(points[idx_i], points[idx_j]) <= radius) {
                hashTable[idx_i].append(idx_j);
                hashTable[idx_j].append(idx_i);
            }
        }
    }
